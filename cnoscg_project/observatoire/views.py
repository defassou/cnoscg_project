...
def recherche_observateur(request):
    observateur = None
    erreur = None
    if request.method == "POST":
        ...
        try:
            observateur = Observateur.objects.get(telephone=telephone)
        except Observateur.DoesNotExist:
            erreur = "Numéro non trouvé"

    return render(request, "observateur/recherche.html", {
...
def recherche_api(request):
    ...
    try:
        ...
        data = {
            "nom": observateur.nom,
            "prenom": observateur.prenom,
            "region": observateur.region,
            "prefecture": observateur.prefecture,
            "commune": observateur.commune,
            "picture": observateur.picture,
        }
        return JsonResponse(data)
    except Observateur.DoesNotExist:
        return JsonResponse({"error": "Numéro introuvable"},status=404)